...
[
  ...
  estUnFeu?       ;; Est-ce que le patch est un feu ?
  estVert?        ;; Est-ce que le feu est vert ?
]
...
to-report road-direct
   ...
   [
      ...
      ask min-one-of banners [distance myself] [
        set X pxcor
        set Y pycor
        ; set qui who
      ]
...
;to avancer
;  ask vehicles [
;    ifelse(not any? other vehicles-on patch-ahead 1) [fd speed][fd 0]
;  ]
;end
...
to setup-feux
  let estIntersection? false
  ask patches with [dir = "N"]
  [
    ask patch-at 0 1 [set estIntersection? intersection?]
    if(estIntersection? = true)
    [
      set pcolor red
      set estUnFeu? true
      set estVert? false
  ...
  ask patches with [dir = "E"]
  [
    ask patch-at 1 0 [set estIntersection? intersection?]
    if(estIntersection? = true)
    [
      set pcolor green
      set estUnFeu? true
...
to changer-feux
  ask patches with [(estVert? = true) or (estVert? = false)]
  [
    ifelse(estVert?) [
      set pcolor red
      set estVert? false
...
to estAuFeu
  ask patch-ahead 1 [
   ifelse((estUnFeu? = true) and (estVert? = false))
   [ask myself [set peutPasser? false]]
   [ask myself [set peutPasser? true]]
  ]
end

to changer-direction
  ask patch-here
  [
    ifelse(intersection?)
    [ask myself[
      set peutChangerDir? true
      if(debug) [show "Je peux tourner"]
    ]]
    [ask myself[
      set peutChangerDir? false
      if(debug) [show "Je ne peux pas tourner"]
    ]]
  ]
  if(peutChangerDir?)
  [
    ...
    [
      ifelse((random 4) <= 3) [set direction 270][set direction 90]
    ]
    if((direction = 90) or (direction = 270))
    [
      ifelse((random 4) <= 3) [set direction 0][set direction 180]
    ]
  ]
end

to avancer
  ifelse(peutPasser? = true) [
    ;let nbPatchesToStop floor acceleration / deceleration
    let car-ahead one-of vehicles-on patch-ahead 1
    ifelse car-ahead != nobody
    [
      ;Les voitures se rentrent dedans
      slow-down-car car-ahead

      ;instant stop sans collision
      ;set speed 0
      ;fd speed
    ]
...
to slow-down-car [ car-ahead ]
  ifelse([ speed ] of car-ahead - deceleration < 0)
  [set speed 0]
